...
export default function DeclaraMEEPPPage() {
    ...
    <div className="min-h-screen bg-slate-100 p-4 md:p-8 text-slate-900 print:bg-white print:p-0">
      ...
      <div className="max-w-[794px] mx-auto mb-6 flex items-center justify-between print:hidden bg-white p-4 rounded-xl shadow-md border border-slate-200">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-bold text-slate-900">Declaração de Enquadramento ME / EPP</h1>
        ...
        <div className="bg-white text-slate-900 font-sans p-10 md:p-14 max-w-[794px] w-full min-h-[1050px] shadow-2xl rounded-sm border border-slate-300 text-sm leading-relaxed flex flex-col justify-between print:shadow-none print:border-none print:w-full print:max-w-none print:min-h-0 print:p-0">
          <div>
            ...
            <div className="space-y-4 text-xs md:text-sm text-slate-900 text-justify leading-relaxed">
              <p>
                A empresa <strong>{company.razaoSocial}</strong>, inscrita no CNPJ sob o nº <strong>{company.cnpj}</strong>, sediada em {company.endereco}, por intermédio de seu representante legal Sr.(a) <strong>{legalResp.nome}</strong>, inscrito(a) no CPF sob o nº <strong>{legalResp.cpf}</strong>{legalResp.rg ? ` e RG nº ${legalResp.rg}` : ""}, <strong>DECLARA</strong>, sob as penas da lei, para fins de participação na licitação <strong>{proposal.numPregao}</strong> perante o <strong>{proposal.orgao}</strong>, que:
              </p>

              <p className="pl-4 border-l-2 border-slate-300 italic">
                1. Cumpre plenamente todos os requisitos legais para enquadramento como <strong>MICROEMPRESA (ME)</strong> ou <strong>EMPRESA DE PEQUENO PORTE (EPP)</strong>, nos termos da Lei Complementar nº 123, de 14 de dezembro de 2006 e suas alterações posteriores;
              </p>

              <p className="pl-4 border-l-2 border-slate-300 italic">
                2. Não se enquadra em nenhuma das hipóteses de vedação previstas no § 4º do artigo 3º da Lei Complementar nº 123/2006, estando apta a usufruir do tratamento favorecido e diferenciado estabelecido nos arts. 42 a 49 da referida lei;
              </p>

              <p className="pl-4 border-l-2 border-slate-300 italic">
                3. Está ciente de que a prestação de falsa declaração sujeitará a empresa e seus responsáveis às sanções administrativas, cíveis e penais cabíveis, especialmente as previstas na Lei nº 14.133/2021 e no art. 299 do Código Penal.
              </p>
            </div>
